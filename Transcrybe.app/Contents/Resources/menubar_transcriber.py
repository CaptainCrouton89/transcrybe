    ...
    def _request_permissions(self):
        ...
        try:
            ...
            try:
                ...
                stream.start()
                stream.read(1)
                permissions_status['microphone'] = True
                logging.info("Microphone access successful")
            except Exception as e:
                permissions_status['microphone'] = False
                logging.warning(f"Microphone access failed: {e}")
            
            # Request accessibility permission by trying to use keyboard controller
            logging.info("Testing accessibility access...")
            try:
                kb = keyboard.Controller()
                # Try a simple key operation
                permissions_status['accessibility'] = True
                logging.info("Accessibility access successful")
            except Exception as e:
                permissions_status['accessibility'] = False
                logging.warning(f"Accessibility access failed: {e}")
            ...
            try:
                ...
                test_hotkeys.start()
                permissions_status['input_monitoring'] = True
                logging.info("Input monitoring access successful")
            except Exception as e:
                permissions_status['input_monitoring'] = False
                logging.warning(f"Input monitoring test failed: {e}")
                
            # Check if any permissions are missing
            missing_permissions = []
            if not permissions_status.get('microphone', False):
                missing_permissions.append("• Microphone access (for recording speech)")
            if not permissions_status.get('accessibility', False):
                missing_permissions.append("• Accessibility access (for pasting text)")
            if not permissions_status.get('input_monitoring', False):
                missing_permissions.append("• Input monitoring (for global hotkey)")
                
            if missing_permissions:
                # Show notification about missing permissions
                missing_text = "\n".join(missing_permissions)
                rumps.notification(
                    "Transcrybe - Permissions Required", 
                    "Missing permissions:",
